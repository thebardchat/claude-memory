...
> Updated: 2026-04-03 | Last session: Agent Ecosystem + Dashboard Overhaul + Neural Map + Buddy v2.0
...
## Session Statistics (as of 2026-04-03)
- 33+ sessions, 680+ hours
- 1,900+ Bash calls, 105+ commits
- 42 MCP tools, 20 services (19 systemd + shanebrain-agents), 7 Docker containers
- 7 AI agents with 50 red line rules
- 4-node cluster, 3 Pico sensors
- 2,893 knowledge objects in Weaviate (20 collections)
...
## MEGA Crew — Live as of April 8, 2026
- 16 persistent Python workers running 24/7 on the Pi
- Location: /mnt/shanebrain-raid/shanebrain-core/mega/bots/
- Systemd service: mega-crew.service (enabled, survives reboots)
- Supervisor: crew_supervisor.py — launches all 16, restarts crashes, writes bot_status.json every 30s
- Message bus: bus.py (SQLite at /mnt/shanebrain-raid/shanebrain-core/mega/bus.db)
- Arc is the sole gatekeeper — NOTHING writes to core files without Arc approval
- Weld is the only bot that executes Arc-approved commits

Zones:
- Brain: Sparky (training mining), Volt (drift detection), Neon (Weaviate embedding), Glitch (adversarial testing)
- Left Hand: Rivet (dedup + format), Torch (prompt suggestions), Weld (commit executor)
- Right Hand: Blaze (context injection), Arc (gatekeeper), Flux (health monitor)
- Left Foot: Bolt (log pattern analysis), Stomp (memory conflict resolution), Grind (bulk re-embedding)
- Right Foot: Crank (scheduler), Spike (IQ benchmarking), Forge (tool stub drafter)

Key files:
- /mnt/shanebrain-raid/shanebrain-core/mega/bot_status.json — live crew status + MEGA_IQ score
- /mnt/shanebrain-raid/shanebrain-core/mega/memory.db — MEGA's conversation memory
- /mnt/shanebrain-raid/shanebrain-core/mega/training.jsonl — growing fine-tune corpus
- /mnt/shanebrain-raid/shanebrain-core/mega/persona.json — MEGA's identity + system prompt
- /mnt/shanebrain-raid/shanebrain-core/tools/pending/ — Forge drops new tool stubs here for Shane review

MEGA model: llama3.1:8b on localhost:11434 (main Ollama, RAID)
